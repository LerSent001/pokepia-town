...
for y in range(256):
 for x in range(256):
  ...
  px.extend((*c,1))
img.pixels=px;img.filepath_raw=str(ROOT/'public/textures/player-plaid.png');img.file_format='PNG';img.save();img.pack();tex=shirt.node_tree.nodes.new('ShaderNodeTexImage');tex.image=img;shirt.node_tree.links.new(tex.outputs['Color'],shirt.node_tree.nodes.get('Principled BSDF').inputs['Base Color'])
